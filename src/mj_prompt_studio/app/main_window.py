...
class MainWindow(QMainWindow):
    ...
    def _build_toolbar(self) -> None:
        ...
        actions = [
            ...
            ("開く", self._info_not_implemented),
            ("保存", self._save_current_document),
            ("エクスポート", self._copy_markdown_record),
    ...
    def _new_project(self) -> None:
        self.project, self.document = self.context.prompt_service.create_project_with_document(
            "New Prompt Project", "Untitled Prompt"
        )
        self._refresh_all()
    ...
    def _handle_job_completed(self, agent_name: str, payload: dict[str, Any]) -> None:
        if agent_name == "IntentIntakeAgent":
            ...
            if isinstance(patches, list) and patches:
                patch = PromptPatch.from_dict(patches[0])
                self.context.prompt_service.apply_patch(self.document, patch)
