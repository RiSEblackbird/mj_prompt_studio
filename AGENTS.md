...
- 作業ブランチを作成して作業をすること。
- 作業ブランチ上で適切にコミット粒度を分けてコミットをしながら進めること。
...
- タスク境界は大きく取る。通常なら 5 スライス程度に分ける規模の作業を、1 つの大きなタスクコミットで完結させる前提で計画する。
...
- 大きなタスクコミットでは、単に差分量を減らすための中途半端な分割をしない。1 つの受け入れ条件を満たす単位で cohesive commit にする。
